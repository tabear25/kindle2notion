...
- **Backend status banner (`checkBackendHealth`)**: `fetch` rejects identically for a sleeping instance, a wrong URL, a missing CORS entry and mixed content, so the banner classifies the failure instead of always printing the wake-up text (which used to stick forever on a misconfiguration):
  ...
  - Network-level rejection: `navigator.onLine === false` -> offline; then the two rejection kinds are split, because they mean opposite things. **Our own timeout** (`withTimeout` tags its abort with `error.timedOut`) means the request was accepted and left unanswered — a booting instance — so it stays "起動中（応答待ち）" within the grace period and becomes "応答がありません" past it, and no second probe is fired (it would hang too). **An outright refusal** runs a `mode: "no-cors"` probe of `/healthz`: an opaque response proves the server answered, so the failure is **CORS** (the message names `location.origin` to add to `CORS_ALLOWED_ORIGINS`); no response means unreachable, "起動中" within the grace period, then "接続できません（N秒経過）"
  - Retry cadence: `HEALTH_RETRY_MS` 5s while waking, `HEALTH_SLOW_RETRY_MS` 15s after a diagnosed fault (so a fixed env var recovers by itself), `0` for a config problem. `HEALTH_TIMEOUT_MS` is **45s** and `WAKE_GRACE_MS` **240s**: Render holds the request open while the instance boots, and this image (Playwright + Chromium + gunicorn) needs well over a few seconds, so a short timeout aborts every wake-up attempt and then misreports a healthy backend as unreachable (that was the 2026-08 regression). Elapsed time is measured from `healthWaitStartedAt`, reset on save so a new URL restarts the grace period
