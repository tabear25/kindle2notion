...
- Pure helpers (`build_notes_from_payload`, `summarize_plan`, `_coerce_books`, `_coerce_highlight`, `normalize_title_for_match`, `find_similar_titles`) are covered by `test/test_add_manual_highlights.py`; heavy deps (`main`, `notion`, `gspread`) are imported lazily inside `main_cli` / `_run_list_books` so the module imports without them. Unicode-sensitive tests use `\uXXXX`-equivalent literals kept ASCII-safe so source encoding can't break them.
- The interactive flow (assistant reconciles the title via `--list-books`, asks for highlights, builds the JSON, dry-runs, confirms, applies) is encoded in the project skill `.claude/skills/adding-manual-highlights/SKILL.md`.
