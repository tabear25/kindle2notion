...
Tests use pytest with monkeypatching. There is no separate test runner script. The test directory is git-ignored, so tests exist only locally. On this Windows machine the `--basetemp` flag is required (the default temp dir is permission-denied). Some tests in `test_split_per_book.py` / `test_add_manual_highlights.py` carry skip guards that date from when the incremental-NotebookLM-merge API lived only on the unmerged `feature/update_split-per-book` branch (commit `f056383`). That branch has since been merged and `merge_notes_into_volume` is in `main`, so those guards no longer skip anything and can be dropped when the tests are next touched.
...
- Basic auth **skips `OPTIONS`** (CORS preflights never carry Authorization); `web/cors.py` answers allowlisted preflights (`CORS_ALLOWED_ORIGINS`, exact-match, unset = no CORS on the API)
- **`/healthz` always answers `Access-Control-Allow-Origin: *`**, independent of `CORS_ALLOWED_ORIGINS` (`web/cors.py`'s `after_request` is registered even with an empty allowlist). It is already unauthenticated and returns a constant body, so it leaks nothing — and being universally readable makes it a trustworthy **liveness oracle** for the frontend. This matters because **Render's router serves its own 502/503 pages while an instance boots, and those carry no CORS headers**: a readable `/healthz` therefore proves the Flask app itself answered, not the edge in front of it. Consequence: cross-origin, the frontend can never read the HTTP status of a cold start, so "is it booting?" must be answered by probing, not by status codes. The `/api/*` allowlist is unchanged and is **not** widened by this
...
- **Backend status banner (`checkBackendHealth`)**: `fetch` rejects identically for a sleeping instance, a wrong URL, a missing CORS entry and mixed content, so the banner classifies the failure instead of guessing:
  ...
  - The probe is a **plain `fetch`, not `apiFetch`** — `/healthz` is auth-exempt server-side, and attaching `Authorization` would make it a preflighted request. The preflight is the first thing to break during a cold start (Render's edge 502 has no CORS headers) and its failure is unreadable from JS, which is exactly how a booting backend used to be reported as dead
  - Response received: `404` -> wrong URL, `502/503/504` -> waking then "起動できていない", other `5xx` -> backend error, anything else -> unexpected response. Cross-origin these fire only if the failing response itself carries CORS headers, which Render's edge pages do not — cold starts land in the network-failure path below
  - Network-level rejection: `navigator.onLine === false` -> offline. **Our own timeout** (`withTimeout` tags its abort with `error.timedOut`) means the request was accepted and left unanswered — a booting instance — so it stays "起動中（応答待ち）" within the grace period, "応答がありません" past it, and fires no second probe. **An outright refusal** runs a `mode: "no-cors"` probe of `/healthz` via `probeReachable()`, which returns **three** states, not a boolean: `"reachable"` (opaque response), `"timeout"` (accepted but hanging), `"unreachable"`. Within the grace period all three mean the same thing and print the wake-up text; past it they split into "アプリが起動していません" (the edge answered, the app never came up), "応答がありません", and "接続できません". ⚠️ **Collapsing these to a boolean was the 2026-08 bug**: a hanging probe on a booting instance was reported as "バックエンドに接続できません（N秒経過）" while the backend was perfectly healthy
  - **CORS is diagnosed by `verifyApiReachable()`, not by the reachability probe.** Since `/healthz` is readable by every origin, an opaque response proves nothing about CORS — so the old "opaque ⇒ CORS" inference was unsound. Instead, *after* `/healthz` returns ok, one `apiFetch("/api/status")` runs: a network-level rejection there is the real signature of a missing `CORS_ALLOWED_ORIGINS` entry (the message names `location.origin`), and a `401/403` is the credentials message. Only then does the banner report success
  - Retry cadence: `HEALTH_RETRY_MS` 5s while waking, `HEALTH_SLOW_RETRY_MS` 15s after a diagnosed fault (so a fixed env var recovers by itself), `0` for a config problem. `HEALTH_TIMEOUT_MS` is **45s** (the main probe doubles as the wake-up request, which Render holds open), while `PROBE_TIMEOUT_MS` is **10s** — the reachability and API probes only ask "did anything answer?" and must not spend the wake-up budget. `WAKE_GRACE_MS` is **420s**: a Playwright + Chromium + gunicorn image on the free plan regularly needs several minutes, and the old 240s expired mid-boot. Elapsed time is measured from `healthWaitStartedAt`, reset on save so a new URL restarts the grace period
  - "バックエンドの状態を確認しています..." is set **only on the first attempt** of a wait; later polls leave the existing banner alone, so the wake-up explanation stays on screen instead of being overwritten every cycle
...
- Files absent from the folder go into `missing_files` (service accounts cannot create Drive files) and their highlights are NOT written. Empty `notes` short-circuits with zero I/O. Returns `{new_books, new_highlights, skipped_duplicates, skipped_invalid, total_notes, missing_files, touched_volumes, index_error}`. Progress uses the existing `"sheets"` phase, one tick per file.
- **`index_error`**: the volumes are written *before* the index, so an API failure during the index refresh loses no highlights — it only leaves the catalogue stale. That case is caught, reported as a non-empty `index_error` string (instead of raising), and recovered with `py -3 -m scripts.split_per_book --apply`. `add_manual_highlights.write_notes` turns it into a `problems` entry, so the CLI exits non-zero and the web API returns `ok: false`.
...
- **Quota handling (rewritten 2026-07-25 after a live 429).** Sheets enforces separate per-minute per-user quotas (60 reads / 60 writes for the service account), and one logical operation costs several requests: gspread's `open_by_key` **and** `Spreadsheet.sheet1` each fetch spreadsheet metadata, so opening a file = 2 reads, `get_all_values()` = 1 read, `clear()`+`update()` = 2 writes. The old fixed per-call sleeps throttled *calls*, not requests, and the writer re-opened each file the reader had just opened — so a sync touching 11 volumes burned ~58 read requests in a burst and died on a 429 with the volumes written but the index stale. Three mechanisms now prevent that, and every API call in this module goes through them:
  1. `_RateLimiter` — sliding 60s window per quota bucket (`_READ_LIMITER` / `_WRITE_LIMITER`, capped at `READ_QUOTA_PER_MINUTE` / `WRITE_QUOTA_PER_MINUTE` = 50, below Google's 60 for headroom). It counts **requests**, not calls, and blocks only as long as needed, so a big run paces itself instead of failing. `acquire()` returns the seconds waited, which is what the tests assert on.
  2. `_open_worksheet()` — caches the opened worksheet per Drive file id for the run (`_WORKSHEET_CACHE`), so a volume that is read *and* rewritten costs 3 read requests instead of 5. `_reset_api_state()` clears it at the start of `sync_notes_to_notebooklm` / `list_books_from_index` / `main_cli`, so the long-lived Flask worker can never reuse a stale handle.
  3. `_with_quota_retry()` — reserves quota, runs the call, and retries a 429 after `QUOTA_RETRY_WAIT_SECONDS` (× `MAX_QUOTA_RETRIES`) as a safety net for quota consumed by another process on the same service account.
  Request costs live in `OPEN_READ_REQUESTS` / `VALUES_READ_REQUESTS` / `VOLUME_WRITE_REQUESTS`; `_quota_eta_seconds()` + `_format_eta()` turn them into the wall-clock estimates the sync and the CLI print before a long run. `_read_volume_throttled` is now just an alias for `_read_volume` (pacing applies to every caller). **There is no manual batching to do** — a caller may pass any number of books in one go; the run gets slower, not broken (verified live 2026-07-25: 20 volumes + index = 63 read requests completed in 70s with one 36s pause).
...
- Pure helpers (`safe_title_for_filename`, `volume_for_book_id`, `volume_filename`, `index_filename`, `all_target_filenames`, `group_books_by_volume`, `volume_rows`, `index_rows`, `group_highlights_by_book`, `_volume_row_to_highlight`, `_index_row_to_book`, `volumes_for_book_ids`, `merge_notes_into_volume`, `merge_summaries`, `plan_redistribution`) and the orchestrator (`sync_notes_to_notebooklm` / `list_books_from_index`, with Drive faked in-memory) are covered by `test/test_split_per_book.py`. `note_utils` is imported unconditionally (pure stdlib); `gspread`/`google-auth` are guarded behind `_RUNTIME_DEPS_OK` so tests import the module without them. The local-only tests were originally written against `VOLUME_COUNT = 49`; the assertions hardcoding a `vol_NN` filename or a layout size of 49/50 were updated to the 99+1 layout on 2026-07-25. Any *new* assertion must not hardcode a book's volume number — derive it from `volume_for_book_id`.
...
- `test/test_split_per_book.py`: tests for `scripts/split_per_book.py` — the pure helpers (filename sanitisation, grouping, row shaping, row↔dict parsers, `merge_notes_into_volume`, `merge_summaries`, `volumes_for_book_ids`) plus the `sync_notes_to_notebooklm` / `list_books_from_index` orchestrator with an in-memory fake Drive (no network), and the quota layer (`_RateLimiter` windowing/eviction, request-not-call counting, `_open_worksheet` caching + `_reset_api_state`, `_with_quota_retry` 429 retry vs. re-raise, ETA helpers, and the `index_error` path). The quota tests use sub-second windows and a `_FakeAPIError`/`_FakeClient` pair rather than patching the clock, so they add well under a second to the suite
...
- **A large sync is slow, not broken.** Every Sheets call is quota-paced, so adding books that land in many different volumes at once makes the run pause (it prints `[quota] read limit reached ...; waiting Ns`) rather than 429. Do not "fix" a slow sync by splitting the input into batches by hand, and do not remove the pacing to speed it up — that is exactly the 2026-07-25 failure mode. A `[warn] ... index refresh failed` line means the highlights are safe in their volumes and only the catalogue needs `py -3 -m scripts.split_per_book --apply`.
