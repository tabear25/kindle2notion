...
`scripts/split_per_book.py` は v2 マスタ (`reading_note`) を読み、
書籍ごとに 1 ファイルずつ Sheets を生成し、`reading_note` と同じ親フォルダの
`per_book/` サブフォルダに置きます。

- ファイル名: `BK-XXXXXX__<書名>` (例: `BK-A2B9B7__GRIT やり抜く力を手に入れる`)
- 1 枚目シート: `highlight_id, location, content` の 3 列
- マスタ側は **読むだけ**、書き換えません
- 既存ファイルは中身をフル再生成 (idempotent)

```bash
python -m scripts.split_per_book           # dry-run (何件 create / update するか表示)
python -m scripts.split_per_book --apply   # 実行
```

実行後、Drive 上の `per_book/` 内のスプレッドシート群を NotebookLM に
ソースとして取り込めばOKです。新しい本を追加したら `--apply` を再実行すれば
差分だけ反映されます。
