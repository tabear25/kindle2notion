...
#### NotebookLM 用に書籍別ファイルへ分割

NotebookLM は各スプレッドシートの 1 枚目のシートしか読み込みません。
`scripts/split_per_book.py` は v2 マスタ (`01_books` + `02_highlights`) を読み、
書籍ごとに 1 ファイルずつ Sheets を生成し、`per_book/` サブフォルダに
中身を書き込みます。マスタ側は **読むだけ**、書き換えません。

- ファイル名: `BK-XXXXXX__<書名>` (例: `BK-A2B9B7__GRIT やり抜く力を手に入れる`、拡張子なし)
- 1 枚目シート: `highlight_id, location, content` の 3 列
- 既存ファイルは中身をフル再生成 (idempotent)
...
2. **必要なファイル名一覧を取得 (dry-run)**
   ...
   `[create ]` 行と末尾の "create them manually" ブロックに、
   作成すべきファイル名がすべて表示されます。

3. **書籍ごとの空 Google Sheets を `per_book/` 内に手動作成**
   - 表示されたファイル名と **完全一致** させる (拡張子なし)
...
4. **書き込み (apply)**
   ...
   既存ファイルだけが上書きされ、まだ作成されていないファイルは
   `[missing]` として警告表示されます。新しい書籍を追加したらそのファイルを
   手動で作って `--apply` を再実行すれば差分だけ反映されます。

実行後、Drive 上の `per_book/` 内のスプレッドシート群を NotebookLM に
ソースとして取り込めば OK です。
...
- 親フォルダの `per_book/` 配下に、dry-run で表示されたファイル名と完全一致する
  Google Sheets を **手動で作成** してから `--apply` を実行する
- 親フォルダ自体を Service Account の編集者権限で共有しておく
  (これによりフォルダ内に作ったシートも自動的に編集可能になる)

詳細は上記「NotebookLM 用に書籍別ファイルへ分割」を参照。
